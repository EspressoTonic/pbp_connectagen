...
sub checkOptions {
    ...
    if($opts{p}) {
        ...
        print "No contig file path argument given.\n";
        help();
    }
...
USAGE
OLD: GBS_PBP-Gene_Typer.pl -1 <forward fastq file: fastq> -2 <reverse fastq file: fastq> -r <reference databases directory: file path> -o <output directory name: string> -n <output name prefix: string>  [OPTIONS]
NEW: GBS_PBP-Gene_Typer.pl -p <contig scaffold file: fastq> -r <reference databases directory: file path> -o <output directory name: string> -n <output name prefix: string>  [OPTIONS]
